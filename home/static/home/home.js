...
function loadContent() {
  ...
  $.getJSON("search/" + search + "/" + contentCount, function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
      $("#content > div:nth-child(2)").append(
        `<span data-id="${data[i].id}" loading="lazy" style="background-image:url('/images/${data[i].media}')"></span>`
      );
...
async function loadGif(e) {
  ...
  if (id) {
    ...
    document
      .querySelector("#gif_content_buttons > button")
    ...
    document.getElementById("gif_tags").addEventListener("click", function (e) {
      ...
      if (tag) {
        ...
        document.querySelector("#content div:first-child").style.display =
          "inline-block";
        document.querySelector("#content div:first-child p").innerHTML =
          tag + " Gifs";
    ...
    relatedGifs.slice(0, 8).forEach((rGif) => {
      if (data.id !== rGif.id) {
        relatedGifContent.insertAdjacentHTML(
          "beforeend",
          `<span data-id="${rGif.id}" loading="lazy" style="background-image:url('/images/${rGif.media}')"></span>`
        );
      }
    });
  }
...
function scrollFunction() {
  ...
  if (
    document.body.scrollTop > Number(navh) + 10 ||
    document.documentElement.scrollTop > Number(navh) + 10
  ) {
    if (isTop == true) {
...
function checkScroll() {
  ...
  if (
    window.scrollY + window.innerHeight >= bodyHeight - 100 &&
    contentCount > 0
  ) {
    if (isBottom == false) {
      isBottom = true;
...
function menu(el) {
  if (el.className == "close") {
    ...
    el.className = "close";
    // document.body.style.overflow = "auto";
    document.getElementById("expage_back").style.transition =
      "opacity 0.2s, z-index 0s 0.2s";
    document.getElementById("expage_back").style.opacity = "0";
    document.getElementById("expage_back").style.zIndex = "-9";
    document.getElementById("expage").style.transition =
      "opacity 0.2s, z-index 0s 0.2s";
...
function srch(el) {
  ...
  document.querySelector("#content div:first-child").style.display =
    "inline-block";
  document.querySelector("#content div:first-child p").innerHTML =
    cat[0].toUpperCase() + cat.slice(1) + " Gifs";
...
document
  .querySelector("#content div:nth-child(2)")
  .addEventListener("click", loadGif);
document.getElementById("gif_related").addEventListener("click", loadGif);
document
  .getElementById("close_gif_view")
  .addEventListener("click", function () {
    document.body.style.overflow = "auto";
    document.getElementById("gif_view").classList.add("gif_view--hidden");
  });
